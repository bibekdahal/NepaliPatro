...
namespace NepaliPatro
{
    ...
    public sealed partial class NotificationsListPage : Page
    {
        ...
        void LoadNotifications()
        {
            ...
            {
                items.Add(new NotificationItem(notifcations[i].DeliveryTime.ToString("D"), notifcations[i].DeliveryTime.ToString("T"),
                    notifcations[i].Content.InnerText));
            }
            DefaultViewModel["Items"] = items;
            if (items.Count == 0)
            {
                sp_remove.Visibility = Windows.UI.Xaml.Visibility.Collapsed;
                tb_empty.Visibility = Windows.UI.Xaml.Visibility.Visible;
            }
            else
            {
                sp_remove.Visibility = Windows.UI.Xaml.Visibility.Visible;
